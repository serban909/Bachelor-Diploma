...
# This code uses COMPARATIVE control with fuzzy logic
...
KP_BASE = 8.0   # Increased for comparative control
KI_BASE = 0.2   # Reduced to prevent overshoot   

ERROR_SMALL = 3.0    
ERROR_MEDIUM = 10.0   
ERROR_LARGE = 25.0   

FUZZY_MIN_OUTPUT = -200 
FUZZY_MAX_OUTPUT = 200  
INTEGRAL_MIN = -100.0    
INTEGRAL_MAX = 100.0     
...
def main():
    ...
    print("="*50)
    print("Configuration:")
    print("  Control mode: FUZZY + Emergency Recovery")
    ...
    print("  Turn rates: Fuzzy-adaptive | Emergency=" + str(EMERGENCY_TURN) + " deg/s")
    ...
    try:
        while True:
            ...
            if left_r < CROSSING_THRESHOLD and right_r >= CROSSING_THRESHOLD:
                ...
                # Invert the sign: when left is darker (negative diff), turn right (positive)
                error = -difference
                turn_rate = fuzzy_compute(error, dt)
