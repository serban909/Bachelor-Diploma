...
\subsection{Robot-side component: WifiStreamer}
\label{sec:wifi-streamer}

To handle the telemetry data streaming from the robot to the host PC, the class\texttt{WifiStreamer} was implemented. This class initializes a network socket using User Datagram Protocol (UDP).
...
\begin{lstlisting}[language=Python, caption={WifiStreamer constructor}, label={lst:lk-init}]
...
\begin{lstlisting}[language=Python, caption={WifiStreamer serializer}, label={lst:lk-init}]
...
The \texttt{sendPacket()} method encodes the telemetry data into a string format and sends it to the PC using the UDP socket. Since Python socket methods can't send pure strings, they serializes alphanumeric strings into UTF-8 byte arrays and this method is the base wrapper that pushes the data onto the network socket layer.

\begin{lstlisting}[language=Python, caption={WifiStreamer algorithm data}, label={lst:lk-init}]
def send(self, step, disturbance, output):
    self.sendPacket(str(step)+","+str(disturbance)+","+str(output)+"\n")
\end{lstlisting}

The \texttt{send()} method is a helper function that formats the step, disturbance and output, either turn rate or speed, from the \texttt{compute()} methods into a comma-separated string with a newline character at the end. This format is parsed on the PC side to be analyzed later.
...
The \texttt{sendCell()} method formats the cell coordinates and wall information into a comma-separated string with a newline character at the end. This method is called by the \texttt{MazeSolver} to send the explored cell information to the PC for visualization.

\subsection{PC-side component: TelemetryViewer}
\label{sec:pc-receiver}

\subsubsection{Graph Window}

The \texttt{GraphWindow} class creates a matplotlib figure in a tkinter window using the \texttt{FigureCanvasTkAgg} class. It displays two animated lines: a red one which represents the disturbance and the input for the algorithm's \texttt{compute()} method and a blue line which represents the output of the algorithm.

\begin{lstlisting}[language=Python, caption={Graph Window}, label={lst:lk-init}]
...
The animation runs at 50 ms interval, having each frame copying the current data from the robot via the \texttt{WifiStreamer} and updating the line objects. The graph always shows the line graph regardless of which task is running, since the labels are automaticallu adjusted through the \texttt{makeLabels()} function to reflect the current task and algorithm.

\subsubsection{Maze Map Window}

The \texttt{MazeMapWindow} class creates a second and optional window to visualize the explored maze map. It receives the cell information from the robot via the \texttt{WifiStreamer} and draws the cells with their walls on the canvas. The current position of the robot is highlighted in blue, the start cell in green, and all other cells in grey. Walls are drawn as white lines on the edge of each cell, using the bitmask from the CELL packets: a set bit means open passage, while a cleared bit means a wall.

\begin{lstlisting}[language=Python, caption={Maze Map Window}, label={lst:lk-init}]
...
The \texttt{LauncherApp} class is the main entry point of the PC application. It creates the interface for selecting the task (Lane Keeping or Maze Solver) and the algorithm (PID, Fuzzy PI or Fuzzy Rule-Based), the EV3 hostname and the maze map checkbox. 

When the user clicks "Start Robot", the launcher performs three actions in sequence:

Firstly, the method \texttt{patchRobotFile()} modifies the two global variables TASK and ALGORITHM in the \texttt{fuzzy\_robot.py} file using regular expressions and patches the \texttt{PC\_IP} address to the current PC's current local IP using a dummy UDP socket.

Secondly, the graph window and optionally the maze map window are opened. 

Finally, the method \texttt{launchEV3()} starts a background thread that uses SCP to copy the patched robot file to the EV3 and SSH to execute it via brickrun. The deployment runs in a Powershell subprocess with a new console window so that the user can see the robot's console output live.
...
The \texttt{Algorithm} abstract base class defines the interface for the control strategies. 

\begin{lstlisting}[language=Python, caption={Algorithm Base Class}, label={lst:algorithm-base}]
class Algorithm:
    def compute(self, setpoint, measuredpoint, dt):
...
It define the \texttt{compute()} method that takes the setpoint, measured point and time delta as input and returns the control output and the \texttt{reset()} method to clear any internal state of the algorithm when starting a new control sequence.

Three concrete subclasses implement this interface:
\begin{itemize}
    \item \texttt{PIDAlgorithm}: Implements a standard Proportional-Integral-Derivative controller.
    \item \texttt{FuzzyPIAlgorithm}: Implements a Fuzzy PI controller with three membership functions and three rules to scale the gains adaptively.
    \item \texttt{FuzzyRuleBased}: Implements a pure Mamdani Fuzzy Logic controller with 15 rules designed for the specific task.
\end{itemize}

The \texttt{Controller} abstract base class defines the interface for the logic execution of a specific task. Both tasks share the algorithm instance, the WiFi streamer, the CSV logger, the EV3 hardware references, the stopwatch, the step counter and the \texttt{algorithmLabel()} method.

\begin{lstlisting}[language=Python, caption={Controller Base Class}, label={lst:controller-base}]
class Controller:
    def __init__(self, algorithm, configuration, streamer, ev3, robot, label):
...
Two concrete subclasses implement the \texttt{run()} method:
\begin{itemize}
    \item \texttt{LaneKeepingController}: Implements the lane keeping logic, reading the left and right sensors to compute the error and using the algorithm to compute the turn rate.
    \item \texttt{MazeSolverController}: Implements the maze solving logic, using the \texttt{MazeMap} class to keep track of the explored maze and making decisions based on the open paths and visited cells. It uses the algorithm to compute the forward speed based on its distance from a wall.
\end{itemize}
...
\begin{itemize}
    \item Single Responsibility Principle: Each class has one reason to change: \texttt{FuzzyRuleBased} computes the output based on fuzzy logic applied on rules, \texttt{MazeMap} manages graph traversal and mapping, \texttt{WifiStreamer} handles network communication and \texttt{FileLogger} handles data logging. No class mixes concerns.
    \item Open/Closed Principle: New algorithms can be added by creating a new subclass of \texttt{Algorithm} without modifying the existing code. The \texttt{FuzzyRuleBased} class was added without changing the \texttt{PIDAlgorithm} or \texttt{FuzzyPIAlgorithm} classes, and the controllers use it. Similarly, new tasks can be added as new subclasses of the \texttt{Controller} without modifying the algorithm classes.
    \item Liskov Substitution Principle: Any \texttt{Algorithm} can replace any other without breaking the controller, because they all satisfy the same \texttt{compute(setpoint, measuredPoint, dt) -> (disturbance, output)}. The maze solver passes the target distance and the measured distance, while the lane keeper passes 0 as set point and the sensor difference and measured point. Both produce a (disturbance, output) tuple that the controller uses without knowing which algorithm generated it.
    \item Interface Segregation Principle: The \texttt{Algorithm} interface exposes only \texttt{compute()} and \texttt{reset()}. It doesn't force all algorithms to implement fuzzy inference or specific methods for the PID. The \texttt{FuzzyRuleBased} class has additional methods (\texttt{fuzzifyDelta}, \texttt{inference()} and \texttt{defuzzification()}) that are internal implemenation details and not part of the public interface.
    \item Dependency Inversion Principle: The controllers depend on the abstract \texttt{Algorithm} interface rather than concrete implementations. The concrete algorithm is selected at startup by the factory method \texttt{createController()} and injected into the controller constructor. The \texttt{Controller} base class receives its label as a parameter rather than reading global variables, ensuring that all dependencies flow inward through the constructor.
\end{itemize}

\subsection{Factory Pattern}
\label{sec:factory}

\begin{lstlisting}[language=Python, caption={Controller Factory}, label={lst:controller-factory}]
class Controller:
...
The \texttt{createController()} method serves as a factory for creating instances of the controllers and algorithms based on the selected task and algorithm name. 
